...
- [ ] **G3 — Contested.** Name the expert, school, or vendor position that rejects it and write their strongest counter in one sentence. If the opposition can't be voiced, it's an unexamined assumption. Unanimity = DOK2 in costume.
  - **Score this row first, and score it by searching.** Look for the field's own name for the phenomenon (catastrophic forgetting, alignment tax, Goodhart's law…). A named phenomenon with a literature is consensus — no amount of G6 evidence converts it into a spike. Then ask whether an existing standard practice exists *because* practitioners already expect this; if so, they do.
  - **The opponent must have something at stake** — a product, a service, a published position. "Some people might disagree" is not a counter.
...
- **Fail signals by gate:** *G2* — the tactic (true, actionable, small). *G3* — consensus mush ("fast is good") or the named phenomenon (the field already has a term for it). *G4* — the stale holy war (argument everyone has heard) or the private revelation (news to the author, textbook to the reader). *G5* — the observation deck (re-describes the world, orders no one) or a surprising *fact* filed as the spike. *G1* — a paragraph where a sentence belongs. *G6* — floating opinion with no receipts, or an asymmetry claim with only one arm in evidence. Plus: a single resolved "answer" where a live dispute belongs.
- **Demote, don't delete.** G2 or G4 failures usually belong a layer down: true-but-small → DOK2/DOK3; well-known → DOK2 by definition.
- **Never merge a G3 failure into a passing spike to save it.** Attaching consensus to a contested claim dilutes the contested one and usually breaks G1. Write the merged sentence out and re-score it; if it reads flabbier than the spike it replaces, demote instead.

## Provenance check (run before scoring, when an evidence base is declared)

Grep every figure in the draft against the declared source. Coherent, confident prose is not evidence that its numbers exist.

- [ ] Every figure appears in the declared evidence base.
- [ ] No figure is quoted for an experiment the source lists as still running.
- [ ] No interval, p-value or σ multiple is attached to a point estimate that has none in the source.
- [ ] No retracted figure survives anywhere — including inside the card that announces its withdrawal.
- [ ] Superseded and current figures are labelled by vintage, never placed side by side as equals.
- [ ] Asymmetry claims have **both** arms in the evidence base.
- **Report the misses as a list, struck from the argument** — not quietly dropped. Say how many and where they were load-bearing.
